...
export function PredictionsView() {
    ...
    // Use selected race data if a race is chosen, otherwise fall back to latest
    const activeQuery = selectedRace ? selectedQuery : latestQuery;
    const { data, isLoading, isError, refetch } = activeQuery;

    const raceLabel = data?.race_key ? formatRaceKey(data.race_key) : '—';
    ...
    return (
        <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6 animate-fade-in">
            ...
            {data && (
                <p className="mt-6 font-body text-xs text-text-muted text-center">
                    Predictions are generated offline after qualifying · XGBoost classifier · Not financial advice
                </p>
            )}
        </div>
    );
